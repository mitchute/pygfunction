...
def time_ClaessonJaved(dt: float, tmax: float, cells_per_level: int = 5) -> np.ndarray:
    """
...
def time_MarcottePasquier(dt: float, tmax: float, non_expanding_cells: int = 48) -> np.ndarray:
    """
    Build a time vector of expanding cell width following the method of
...
def time_geometric(dt: float, tmax: float, Nt: int) -> np.ndarray:
    """
